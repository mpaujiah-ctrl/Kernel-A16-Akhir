#!/bin/bash
#
# clean_ksun_keep_susfs.sh
#
# Purpose:
#   Remove the existing KernelSU / KernelSU-Next (KSUN) integration while
#   deliberately preserving the existing SUSFS integration.
#
# Intended transition:
#   KSUN      -> REMOVE
#   SUSFS     -> KEEP
#   SukiSU    -> ADD LATER
#
# Derived from the previously used clean_hook_2.sh.
#
...
# IMPORTANT:
# SUSFS is intentionally preserved.
# We do NOT run the old SUSFS cleanup list here, and we do NOT delete:
#   fs/susfs.c
#   fs/sus_su.c
#   include/linux/susfs.h
#   include/linux/susfs_def.h
# We only remove KernelSU/KSUN hooks from KSU_CLEAN_FILES above.

rm -f "$STRIPPER"

echo
echo "[*] Verifying SUSFS files are still present..."
for file in "${SUSFS_REMAIN_CLEAN_FILES[@]}"; do
    if [ -f "$file" ]; then
        echo "[+] Preserved ${file}"
    else
        echo "[!] SUSFS file missing: ${file}"
    fi
done

echo
echo "[*] Checking for remaining CONFIG_KSU references..."
found_ksu=0
for file in "${KSU_CLEAN_FILES[@]}"; do
    [ -f "$file" ] || continue
...
if [ "$found_ksu" -eq 0 ]; then
    echo "[+] No CONFIG_KSU references remain in KSU hook files."
fi

echo
echo "[*] SUSFS references are intentionally retained."
echo "[+] KSUN cleanup completed; SUSFS was NOT removed."
